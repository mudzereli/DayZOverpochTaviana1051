...
_cbank 	= player getVariable ["bank",0];
_cwealth 	= player getVariable ["wealth",0];

diag_log format ["HumanityMorph: Wealth: %1, Bank: %2", _cwealth, _cbank];
if (typeOF player == _model) exitWith {cutText ['You already wear this Skin!', 'PLAIN'];};




PlayerChangingSkins = true;
...
_oldbank 	= player getVariable ["bank",0];
_oldwealth 	= player getVariable ["wealth",0];
_oldwealth = if (typeName _oldwealth == "STRING") then { parseNumber _oldwealth } else { _oldwealth };
_oldbank = if (typeName _oldbank == "STRING") then { parseNumber _oldbank } else { _oldbank };
...
player setVariable["wealth",_oldwealth,true];
player setVariable["bank",_oldbank,true];
